class DispProxToVal {
    constructor(selector, width = 50, height = 52) {
        ...
        this.width = width;
        this.height = height;
        this.svg = d3.select(selector).attr("width", this.width).attr("height", this.height);
    ...
    processData(jsonData) {
        const qualityEachMultiplet = jsonData.spinFitVariableArray.qualityEachMultiplet;
        return qualityEachMultiplet.filter(item =>
            item.containsSigma === true && 'm5NScalPro' in item && 'proj2' in item && 'labelVarSet' in item
        ).map(item => ({
    ...
    getColorFromMap(expVal, index = 0) {
        ...
        const cR = this.interpolate(colorStart.substring(1, 3), colorEnd.substring(1, 3), adjust);
        const cG = this.interpolate(colorStart.substring(3, 5), colorEnd.substring(3, 5), adjust);
        const cB = this.interpolate(colorStart.substring(5, 7), colorEnd.substring(5, 7), adjust);
    ...
    plot01(refposX = 0, refposY = 0, ratio = 0.9937, is1Max = true) {
        ...
        this.drawFrame(refposX, refposY, ratioDispData.length * width, heightCurBlockC, !is1Max);
    }
 ...
 draw() {
        ...
        this.data.forEach((entry, i) => {
            ...
            let textElem = this.svg.append("text")
                .attr("x", posX)
                .attr("y", 10)
                .text(labelVarSet)
                .attr("font-size", "10px")
